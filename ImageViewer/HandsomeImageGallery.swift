...
public class HandsomeImageGallery: UICollectionViewController {
    ...
    public override func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        if let image: UIImageView = cell.viewWithTag(2222) as? UIImageView {
            image.image = nil
            image.contentMode = .scaleAspectFill
            image.image = UIImage(named: self.imageArray[indexPath.row])
        }
        return cell
    }
...
extension HandsomeImageGallery: IndexPathFromDismissedVw{
    func dismissClosure(fromView: UIView?, _ fromFrame: CGRect) {
        guard let frmView = fromView else {
            let cell2 = self.collectionView?.cellForItem(at: self.currentIndexPath!)
            cell2?.isHidden = false
            if let nextIndx = self.indexPathFromDismissed{
                let cell2 = self.collectionView?.cellForItem(at: nextIndx)
                cell2?.isHidden = false
        ...
        if let frm = cellFrameInSuperVw{
            self.nextViewController!.dismissOverlay(parentViewController: self, fromView: frmView, toView: self.view, fromFrame: fromFrame, toFrame: frm, completion: {
                if self.indexChanged{
                    ...
                    let cell = self.collectionView?.cellForItem(at: self.currentIndexPath!)
                    cell?.isHidden = false
                }
                self.indexChanged = false
                self.indexPathFromDismissed = nil
    ...
    func unHideIndexPath(indexPth: IndexPath) {
        if !self.indexChanged{
            if let current = self.currentIndexPath{
                let cell = self.collectionView?.cellForItem(at: current)
                cell?.isHidden = false
            }
        }
        let cell2 = self.collectionView?.cellForItem(at: indexPth)
    ...
    func hideIndexPath(indexPth: IndexPath, _ shouldScroll: Bool) {
        print("-Hide")
        self.indexPathFromDismissed = indexPth
        let cell2 = self.collectionView?.cellForItem(at: indexPth)
        cell2?.isHidden = true
        if self.indexChanged{
            if shouldScroll{
                self.collectionView?.scrollToItem(at: indexPth, at: .centeredVertically, animated: false)
            }
        }
    }
